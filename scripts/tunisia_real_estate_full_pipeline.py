...
FILES = {
    ...
    'mubawab': 'tunisia_estate_complete_clean.csv'
}
OUTPUTS = {
...
for source_name, filename in FILES.items():
    if Path(filename).exists():
        ...
        df_temp['source'] = source_name  # SCALAR - auto-broadcasts to all rows ✅
        dfs.append(df_temp)
...
# Extract price: digits only, robust
df['price_numeric'] = (df['price_str']
    .str.findall(r'\b\d{1,3}(?:,\d{3})*(?:\.\d+)?\b')
    .str[0] if df['price_str'].str.contains(r'\d') else np.nan
)
df['price_numeric'] = (df['price_numeric']
    .str.replace(',', '')
    .astype(float)
)
