...
Path("output/visuals").mkdir(exist_ok=True, parents=True)
...
def load_all_data():
    files = {
        ...
        'mubawab_sales': 'data\\raw\\mubawab_sales_full.csv',
        'mubawab_rentals': 'data\\raw\\mubawab_rentals_full.csv'
    }
    sources = {}
...
def clean_estate_data(df):
    ...
    if size_cols:
        ...
        size_text = pd.Series([''] * len(df_clean), index=df_clean.index)  # <-- FIX
